---
name: compactar-diario
description: Compactación diaria event-driven del second brain — actualiza `## Interacciones` de personas (1-1s + Slack curado), sincroniza `estado` de proyectos con Jira, mantiene `## PRs recientes` en páginas de proyecto, y stagea candidatos a promoción en `candidatos-gold/` para revisión humana. Sin resúmenes temporales (eso es `/compactar semanal|mensual`, en la skill `compactar`). Usar dentro de `/refresh` y `/pipeline-diario`, o para "promover gold diario".
---

# compactar-diario — promoción event-driven a gold (modo diario)

Hace SOLO la mitad **event-driven** de la promoción a gold. **No** genera `resumenes/`. El pipeline
diario lo invoca entre `/todos` y `/backup` como paso best-effort. Los resúmenes semanal/mensual y el
drenado de `candidatos-gold/` viven en la skill `compactar` (`/compactar semanal|mensual`).
